...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 flex flex-col overflow-hidden bg-background" style={{ height: '100dvh' }}>
      <main className="flex-1 overflow-y-auto no-scrollbar relative" style={{ paddingBottom: 'calc(80px + max(6px, env(safe-area-inset-bottom, 6px)))' }}>
        {/* Bottom fade indicator */}
        <div className="pointer-events-none fixed left-0 right-0 h-8 bg-gradient-to-t from-background to-transparent z-10" style={{ bottom: 'calc(68px + max(6px, env(safe-area-inset-bottom, 6px)))' }} />
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 safe-bottom">
        <div className="mx-auto max-w-lg px-4 pb-1" style={{ paddingBottom: 'max(6px, env(safe-area-inset-bottom, 6px))' }}>
